...
@everywhere function Bandit_tune(yyy)
	...
	@everywhere function bandit(num_iter::Int64, boost::Bool,boost_func,q,nn,Y,sigma,xxx,yyy)
		...
		for i=1:num_iter
			...
			w = rand(Distributions.Bernoulli(0.01))*k[1]
...
xxx=logspace(-3,-0.1,6)
yyy=linspace(0.2,1.5,6)
#out_y = pmap(Bandit_tune, xxx)
	
#save("Documents/ANIS/regret_y.jld","out_y",out_y)

using PyPlot
using JLD
out_y = load("/homes/xlu/Documents/ANIS/regret_y.jld")["out_y"]
out_x = load("/homes/xlu/Documents/ANIS/regret_x.jld")["out_x"]
...
for i=1:6
	yyyy = floor(yyy[i],4)
	plot(cumsum(out_y[i]),label="log(i^0.2)/n^$yyyy")
end
legend(loc="best")
